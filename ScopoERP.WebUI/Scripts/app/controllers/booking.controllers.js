var app = angular.module("booking.controllers", [
    "ngGrid",
    "ui.bootstrap",
    "globalModule",
    "booking.services",
...
app.controller('bookingController', ['$scope', '$modal', 'bookingSearchService', 'bookingCrudService', 'toasterService', 'dropdownService', function ($scope, $modal, bookingSearchService, bookingCrudService, toasterService, dropdownService) {
    ...
    $scope.colDefs = [
        ...
        { field: 'Action', cellTemplate: '<button ng-click="unmapBooking(row)">Unmap</button>', enableCellEdit: false }
    ];
        ...
        bookingSearchService.getBookingFromWorksheet($scope.searchCriteria.multiplePO.join(','), $scope.searchCriteria.item).then(function (res) {
            if (res.data.length) {
                ...
                angular.forEach($scope.bookingGridData, function (row) {
                    ...
                    $scope.grandTotalPrice += (row.TotalQuantity * row.UnitPrice);
    ...
    $scope.$on('ngGridEventEndCellEdit', function (event) {
        ...
        $scope.grandTotalPrice = 0;
        for (var i in $scope.bookingGridData) {
            $scope.grandTotalPrice += Number($scope.bookingGridData[i].TotalQuantity) * Number($scope.bookingGridData[i].UnitPrice);
    ...
    $scope.createBooking = function () {
        ...
        bookingCrudService.createBooking(JSON.stringify($scope.bookingGridData)).then(function (res) {
            ...
            if (res.data === "false") {
                ...
                dropdownService.getReferenceNoDropDown().then(function (res) {
                    $scope.searchCriteria.piDropDown = res.data;
                    $scope.searchBooking.piID = '';
                });
            }
        });
    };

    $scope.unmapBooking = function (row) {
        toasterService.wait();
        bookingCrudService.unmapBooking(row.entity.BookingID).then(function (res) {
            if (res.data === "false") {
                toasterService.clear();
                toasterService.error();
            } else {
                toasterService.clear();
                toasterService.customSuccess(res.data.successMsg);
                $scope.refernceNoDropDownChange();
            }
        });
    };

    $scope.clickAllPurchaseOrder = function () {
        var cmbMultiplePO = $('#cmbMultiplePO');
        if (!$scope.allPO.isChecked) {
            cmbMultiplePO.find('option').prop("selected", "selected");
            cmbMultiplePO.find('option').trigger("change");
        } else {
            cmbMultiplePO.find('option').removeAttr("selected");
            cmbMultiplePO.find('option').trigger("change");
        }
    };
